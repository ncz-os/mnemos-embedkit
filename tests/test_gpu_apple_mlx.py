...
from embedkit.adapters.gpu_apple_mlx import AppleMLXAdapter

AVAILABLE = AppleMLXAdapter.is_available()[0]
MODEL_ENV = "EMBEDKIT_TEST_APPLE_MLX_MODEL"


def _model_path() -> str:
    value = os.environ.get(MODEL_ENV)
    if not value:
        pytest.skip(f"set {MODEL_ENV} to run apple-mlx adapter smoke tests")
...
def test_is_available_safe() -> None:
    ...
    for _ in range(1000):
        ok, reason = AppleMLXAdapter.is_available()
...
def test_embed_dim_consistent() -> None:
    adapter = AppleMLXAdapter(_model_path(), max_tokens=128)
    try:
